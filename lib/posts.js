...
export function getAllPosts() {
  ...
  // Newest real post first. Posts carrying a date that has not arrived yet
  // come from the publish-order plan; they stay reachable at their URLs (no
  // indexed page dies), but they sort BEHIND every already-published post
  // instead of in front. Before this, a post dated November outranked a post
  // published today, and the featured slot showed a date from the future.
  const now = Date.now()
  const isFuture = p => new Date(p.date).getTime() > now
...
export function getFeaturedPost() {
  // The newest post whose date has actually arrived.
  const now = Date.now()
  return getAllPosts().find(p => new Date(p.date).getTime() <= now) || null
}
