...
export async function POST(request) {
  try {
    ...
    // Owner notification. Awaited: an un-awaited send dies when the
    // serverless function freezes after the response returns. The try/catch
    // keeps a notification failure from ever failing the member's signup,
    // and the client fires this route without waiting, so the extra
...
<p><a href="${appUrl()}/discover">See who's in the loft</a></p>`,
      ...
      console.error('Owner signup notification failed:', e)
    }
