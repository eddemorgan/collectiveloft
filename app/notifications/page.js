  ...
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <div style={{
        ...
        background: 'var(--bg1)',
        border: '0.5px solid rgba(240,236,227,0.07)',
        borderRadius: '4px',
      ...
      >
        ...
        <div style={{
          ...
          background: iconColor || 'rgba(201,168,76,0.15)',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem', flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'var(--sans)', fontSize: '0.78rem', fontWeight: 600, color: 'var(--cream)' }}>
              {title}
            </span>
            {tag && (
              <span style={{
                ...
                color: tagColor || 'var(--gold)',
                background: tagColor ? `${tagColor}22` : 'rgba(201,168,76,0.12)',
          ...
          <div style={{ fontFamily: 'var(--sans)', fontSize: '0.72rem', color: 'rgba(240,236,227,0.5)', lineHeight: 1.5 }}>
            {sub}
          </div>
        </div>
...
function SectionLabel({ label, count }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: 'var(--sans)', fontSize: '0.58rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(240,236,227,0.3)' }}>
        {label}
      </span>
      {count > 0 && (
        <span style={{
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // 1. Terms waiting for my approval
      ...
      for (const t of pendingTerms || []) {
        const name = t.initiator ? `${t.initiator.firstname} ${t.initiator.lastname}` : 'Someone'
        actions.push({
          key: `terms-${t.id}`,
          icon: '📋',
          iconColor: 'rgba(201,168,76,0.18)',
          title: `${name} sent you collab terms`,
          sub: t.project_title ? `Project: ${t.project_title}` : 'Review the terms and open your Loft Studio.',
      ...
      if (myBriefs && myBriefs.length > 0) {
        const briefMap = Object.fromEntries(myBriefs.map(b => [b.id, b.title]))
        ...
        for (const a of apps || []) {
          ...
          const title = briefMap[a.brief_id] || 'your brief'
          updates.push({
            key: `app-${a.id}`,
            icon: '✉',
      ...
      // 4. Active studios (recently opened)
    ...
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '2.5rem 1.5rem', width: '100%', flex: 1 }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <div style={{ fontFamily: 'var(--sans)', fontSize: '0.58rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold)', opacity: 0.75, marginBottom: '0.4rem' }}>
            Collective Loft
          </div>
          <div style={{ fontFamily: 'var(--serif)', fontSize: '2rem', fontWeight: 700, color: 'var(--cream)', marginBottom: '0.3rem' }}>
            Message Center
          </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: 'rgba(240,236,227,0.25)', fontFamily: 'var(--sans)', fontSize: '0.78rem' }}>
            ✦ Loading…
          </div>
        ) : totalCount === 0 ? (
          <div style={{
            textAlign: 'center', padding: '4rem 2rem',
            background: 'var(--bg1)', border: '0.5px solid rgba(240,236,227,0.06)',
            borderRadius: '6px',
          }}>
            ...
            <div style={{ fontFamily: 'var(--sans)', fontSize: '0.72rem', color: 'rgba(240,236,227,0.35)', lineHeight: 1.65 }}>
              No pending terms, ratings, or applications right now. When something needs your attention, it'll show up here.
            </div>
          </div>
        ) : (
          <>
